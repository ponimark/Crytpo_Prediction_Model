...
def startup_load():
    print("🔄 Loading incremental models and latest windows...")
    ...
    for ticker in config.CRYPTOS:
        ...
        model_path = f"{MODEL_DIR}/{safe}_patchtst_7d_incremental.pt"
        if not os.path.exists(model_path):
            print(f"⚠ Missing incremental model: {safe}")
            continue
        ...
        print(f"✔ Loaded incremental model for {safe}")
        ...
        if len(df) < INPUT_WINDOW:
            print(f"⚠ Not enough processed rows for {safe}")
            continue
...
def predict_single(ticker: str):
    ...
    # Build the 256 x 5 feature tensor
    feats = df[
    ...
    return {
        ...
        "predicted_price_7d": round(float(predicted_price), 4),
        "percent_change": round(float(pct), 4),
        "direction": direction
    }
...
@app.get("/")
def home():
    return {"message": "PatchTST incremental predictor running!"}
...
if __name__ == "__main__":
    print("🚀 Starting FastAPI (incremental models)...")
